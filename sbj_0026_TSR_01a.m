...
[status, cmdout] = system('uname');
if strip(cmdout) == "Darwin"
    dataFolder = "/users/terry/OneDrive - Medical University of South Carolina/Suresh, Rishishankar's files - ECOG + DBS (PD)/data/ECOG_analysis/Unprocessed_data/pro00073545_0026/ecog/VRP_26-210223-160648";
else
    dataFolder = "C:\Users\roddeyt\OneDrive - Medical University of South Carolina\Suresh, Rishishankar's files - ECOG + DBS (PD)\data\ECOG_analysis\Unprocessed_data\pro00073545_0026\ecog\VRP_26-210223-160648";
end
        data.cfg.trial_data.t1.filename=dataFolder;
% freq bands
...
%look at beta
data.trials.t1.epochs.e1(1,1)=1.03e5; %No movement no stim
data.trials.t1.epochs.e1(1,2)=1.29e5; 
data.trials.t1.epochs.e2(1,1)=1.63e5; %No stim R Arm Flex
data.trials.t1.epochs.e2(1,2)=1.90e5;
data.trials.t1.epochs.e3(1,1)=1.90e5; %No stim L Arm Flex
data.trials.t1.epochs.e3(1,2)=2.20e5; 
data.trials.t1.epochs.e4(1,1)=2.20e5; %No stim Post Move
data.trials.t1.epochs.e4(1,2)=2.39e5;
data.trials.t1.epochs.e5(1,1)=2.79e5; %2.0 Anodal Pre Move
data.trials.t1.epochs.e5(1,2)=3.38e5;
data.trials.t1.epochs.e6(1,1)=3.4e5; %2.0 Anodal RA Flex
...
data.trials.t1.epochs.e8(1,2)=4e5;
data.trials.t1.epochs.e9(1,1)=4.56e5; % No stim Pre Move
data.trials.t1.epochs.e9(1,2)=4.96e5;
data.trials.t1.epochs.e10(1,1)=4.96e5; %no Stim RA Flex
data.trials.t1.epochs.e10(1,2)=5.15e5;
data.trials.t1.epochs.e11(1,1)=5.15e5; %No Stim LA Flex
data.trials.t1.epochs.e11(1,2)=5.34e5;
data.trials.t1.epochs.e12(1,1)=5.875e5; %No Stim Post Move
data.trials.t1.epochs.e12(1,2)=6.11e5; 
% data.trials.t1.epochs.e13(1,1)=e5; %2.0 Cat stim Pre Move
% data.trials.t1.epochs.e13(1,2)=3.8e5;
% data.trials.t1.epochs.e14(1,1)=8.07e5; %2.0 Cat Stim RA Flex
% data.trials.t1.epochs.e14(1,2)=8.28e5;
% data.trials.t1.epochs.e15(1,1)=8.28e5; %2.0 Cat Stim LA Flex
% data.trials.t1.epochs.e15(1,2)=4.70e5;
% data.trials.t1.epochs.e16(1,1)=4.705e5; %2.0 Cat Stim Post Move
% data.trials.t1.epochs.e16(1,2)=4.8e5; 
% data.trials.t1.epochs.e17(1,1)=3.5e5; % No stim Pre Move
% data.trials.t1.epochs.e17(1,2)=3.8e5;
% data.trials.t1.epochs.e18(1,1)=4.25e5; %no Stim RA Flex
% data.trials.t1.epochs.e18(1,2)=4.55e5;
% data.trials.t1.epochs.e19(1,1)=4.57e5; %No Stim LA Flex
% data.trials.t1.epochs.e19(1,2)=4.70e5;
% data.trials.t1.epochs.e20(1,1)=4.705e5; %No Stim Post Move
% data.trials.t1.epochs.e20(1,2)=4.8e5; 
% 
...
data.trials.t1.sig.ecog = [ch1, ch2, ch3, ch4, ch5, ch6];
...
%% plots
%try data = data_pro00073545_s026; catch; end
...
%%
data_pro00073545_s026=data;
save('data_pro00073545_026','data_pro00073545_s026')
...
stim = {"Pre Stim", "2.0 mA Anodal", "Post 2.0 mA Andoal"};
section = {1:4, 5:8, 9:12};
...
for ch=1:6
    for i=1:6
        ...
        data.trials.t1.psd.(resultarr(10)).sai.means.(lower(freq))(:, ch)';...
        data.trials.t1.psd.(resultarr(11)).sai.means.(lower(freq))(:, ch)';...
        data.trials.t1.psd.(resultarr(12)).sai.means.(lower(freq))(:, ch)';...
        };
        graphPadExport{1} = char([graphPadExport{1} repmat("", 1, 59-length(graphPadExport{1}))]);
        graphPadExport{2} = char([graphPadExport{2} repmat("", 1, 59-length(graphPadExport{2}))]);
        graphPadExport{3} = char([graphPadExport{3} repmat("", 1, 59-length(graphPadExport{3}))]);
        graphPadExport{4} = char([graphPadExport{4} repmat("", 1, 59-length(graphPadExport{4}))]);
        graphPadExport{5} = char([graphPadExport{5} repmat("", 1, 59-length(graphPadExport{5}))]);
        graphPadExport{6} = char([graphPadExport{6} repmat("", 1, 59-length(graphPadExport{6}))]);
        graphPadExport{7} = char([graphPadExport{7} repmat("", 1, 59-length(graphPadExport{7}))]);
        graphPadExport{8} = char([graphPadExport{8} repmat("", 1, 59-length(graphPadExport{8}))]);
        graphPadExport{9} = char([graphPadExport{9} repmat("", 1, 59-length(graphPadExport{9}))]);
        graphPadExport{10} = char([graphPadExport{10} repmat("", 1, 59-length(graphPadExport{10}))]);
        graphPadExport{11} = char([graphPadExport{11} repmat("", 1, 59-length(graphPadExport{11}))]);
        graphPadExport{12} = char([graphPadExport{12} repmat("", 1, 59-length(graphPadExport{12}))]);
        
        
        writecell(graphPadExport', ['~/MATLAB-Drive/GraphPad/sbj_0026_' num2str(k)]);
    end
end
...
%%
movement = {1:4:12, 2:4:12, 3:4:12, 4:4:12};
    ...
    ep = movement{epselect};
     for j=1:length(freqs)
        ...
        subplot(3,2,ch)
            ...
            grping = [repmat("a", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr(1)).sai.means.(lower(freq))(:, ch), 1))...
                repmat("b", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr(2)).sai.means.(lower(freq))(:, ch), 1))...
                repmat("c", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr(3)).sai.means.(lower(freq))(:, ch), 1))...
];
            epoch_stats_gr_all_s_m = [data_pro00073545_s026.trials.t1.psd.(resultarr(1)).sai.means.(lower(freq))(:, ch)'...
            data_pro00073545_s026.trials.t1.psd.(resultarr(2)).sai.means.(lower(freq))(:, ch)'...
            data_pro00073545_s026.trials.t1.psd.(resultarr(3)).sai.means.(lower(freq))(:, ch)'...
];
            ...
            eval(sprintf('data_pro00073545_s026.stats.psd.test.kw.mc.kwmultoutput_c%d_e%d_%s=multcompare(stats, ''ctype'', ''bonferroni'', ''Display'', ''off'');', ch, epselect, freq));


            hold on
            bar([data_pro00073545_s026.plots.(lower(freq)).(chname).supermeans(ep)], 'FaceColor', 'flat');
            lines = kw_mult_output(find(kw_mult_output(:,6)<0.05), :);
            for i=1:size(lines, 1)
                line = lines(i, :);
                sigstar([line(1) line(2)], line(6));
            ...
            %chname
            errorbar(data_pro00073545_s026.plots.(lower(freq)).(chname).supermeans(ep),data_pro00073545_s026.plots.(lower(freq)).(chname).ses(ep), '.k')
                       %ylabel([freq ' Power']);
                       %xlabel('Epoch');
        ...
        %     subplot(3,2,ch);
        %     hold on
        %     scatter(ep, [data_pro00073545_s026.plots.(lower(freq)).(chname2).supermeans(ep)], "filled");
        %     mdl = fitlm(ep, [data_pro00073545_s026.plots.(lower(freq)).(chname2).supermeans(ep)]);
        %     plot(mdl);
...
%%
table = ep; [data_pro00073545_s026.plots.(lower(freq)).(chname).supermeans(ep(2:2:11))];
%%
...
for j=1:length(freqs)
    ...
    for ch=1:6
        ...
        subplot(3,2,ch)
        hold on
        scatter(ep(1:2:3), [data_pro00073545_s026.plots.(lower(freq)).(chname).supermeans(ep(1:2:11))], "filled");
        mdl = fitlm(ep(1:2:3), [data_pro00073545_s026.plots.(lower(freq)).(chname).supermeans(ep(1:2:11))]);
        plot(mdl);
...
%bar([data_pro00073545_s026.plots.(lower(freq)).(chname).supermeans(1:33)], 'FaceColor', 'flat');
...
for j=1:length(freqs)
    ...
    for ch=1:6
        ...
        originalArray = [data_pro00073545_s026.plots.(lower(freq)).(chname).supermeans(1:12)];
        newLength = 15;
        ...
        labels = ["Pre-Stimulation", "2.0 mA", "Post 2.0 mA"];
        for l=1:3
            grping = [repmat("a", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr((4*l)-3)).sai.means.(lower(freq))(:, ch), 1))...
            repmat("b", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr((4*l)-2)).sai.means.(lower(freq))(:, ch), 1))...
            repmat("c", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr(4*l-1)).sai.means.(lower(freq))(:, ch), 1))...
            repmat("d", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr(4*l)).sai.means.(lower(freq))(:, ch), 1))...
            ];
            epoch_stats_gr_all_s_m = [data_pro00073545_s026.trials.t1.psd.(resultarr((4*l)-3)).sai.means.(lower(freq))(:, ch)'...
                data_pro00073545_s026.trials.t1.psd.(resultarr((4*l)-2)).sai.means.(lower(freq))(:, ch)'...
            data_pro00073545_s026.trials.t1.psd.(resultarr((4*l)-1)).sai.means.(lower(freq))(:, ch)'...
            data_pro00073545_s026.trials.t1.psd.(resultarr(4*l)).sai.means.(lower(freq))(:, ch)'...
            ];
        ...
        %chname
        errorbar(((l*5)-4):l*5-1, data_pro00073545_s026.plots.(lower(freq)).(chname).supermeans(((l*4)-3):l*4),data_pro00073545_s026.plots.(lower(freq)).(chname).ses(((l*4)-3):l*4), '.k')
               %ylabel([freq ' Power']);
               %xlabel('Epoch');
               %title(['Channel ' num2str(ch) ' '  freq ' Power '  char(name{epselect})]);
...
%%
    ...
    % Channel Chooser
stim = {"Stim 1 Movement 1", "Stim 1 Movement 2", "Post Stim 1"};
freqs = {...
    'Delta',...
    'Theta',...
    'Alpha',...
...
for j=1:length(freqs)
    ...
    for ch=1:6
        ...
        origIdx = 1;
        for newIdx = 1:newLength
        if mod(newIdx, 5) == 0 % If it's a fifth index (3, 6, 9, ...)
            newArray(newIdx) = numberToInsert;
        else
            newArray(newIdx) = originalArray(origIdx);
            origIdx = origIdx + 1;
        ...
        labels = ["Pre-Movement", "Right Arm Flexion", "Left Arm Flexion", "Post Movement"];
        for l=1:4
            grping = [repmat("a", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr(l)).sai.means.(lower(freq))(:, ch), 1))...
            repmat("b", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr(l+4)).sai.means.(lower(freq))(:, ch), 1))...
            repmat("c", 1, size(data_pro00073545_s026.trials.t1.psd.(resultarr(l+8)).sai.means.(lower(freq))(:, ch), 1))...
            
            ];
            epoch_stats_gr_all_s_m = [data_pro00073545_s026.trials.t1.psd.(resultarr(l)).sai.means.(lower(freq))(:, ch)'...
            data_pro00073545_s026.trials.t1.psd.(resultarr(l+4)).sai.means.(lower(freq))(:, ch)'...
            data_pro00073545_s026.trials.t1.psd.(resultarr(l+8)).sai.means.(lower(freq))(:, ch)'...
            ...
            hold on
            bar(((l*4)-3):l*4-1,[newArray(l:5:15)], 'FaceColor', 'flat');
            %bar.Cdata(2, :) = [0 0.5 0];
            lines = kw_mult_output(find(kw_mult_output(:,6)<0.05), :);
        for i=1:size(lines, 1)
            line = lines(i, :);
        ...
        %chname
        errorbar(((l*4)-3):l*4-1, data_pro00073545_s026.plots.(lower(freq)).(chname).supermeans(l:4:12),data_pro00073545_s026.plots.(lower(freq)).(chname).ses(l:4:12), '.k')
               %ylabel([freq ' Power']);
               %xlabel('Epoch');
               %title(['Channel ' num2str(ch) ' '  freq ' Power '  char(name{epselect})]);
        ...
        xlabel("Epoch")
        ylabel("Power")
        sgtitle("Patient " + data_pro00073545_s026.cfg.info.n_sbj + " " + freq);
        xticks(2:4:16);
        xticklabels(labels);
        xlim([0 16]);
...
eps = [1 4 10 16 22 28];
    for i=1:6
        subplot(6,2,(2*i-1))
        hold on
        eval("plot(data_pro00073545_s026.trials.t1.sig.ecog(data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)) + "(1,1):data_pro00073545_s026.trials.t1.epochs.e" + num2str(eps(i)+2) + "(1,2),3))");
        xline(eval("data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)) + "(1,2)-data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)) + "(1,1)"), "--b");
        xline(eval("data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)+1) + "(1,1)-data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)) + "(1,1)"), "--g");
        xline(eval("data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)+1) + "(1,2)-data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)) + "(1,1)"), "--g");
        xline(eval("data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)+2) + "(1,1)-data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)) + "(1,1)"), "--r");
        xline(eval("data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)+2) + "(1,2)-data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)) + "(1,1)"));

        hold off
        subplot(6,2,(2*i));
        try eval(["spectrogram(data_pro00073545_s026.trials.t1.sig.ecog(data_pro00073545_s026.trials.t1.epochs.e"+ num2str(eps(i)) + "(1,1):data_pro00073545_s026.trials.t1.epochs.e" + num2str(eps(i)+2) + "(1,2),3),512,[],1024,1000)"]);  catch; close(gcf); continue; end
        view(90, 90);
...
spectrogram(data_pro00073545_s026.trials.t1.sig.ecog(data_pro00073545_s026.trials.t1.epochs.e16(1,1):data_pro00073545_s026.trials.t1.epochs.e18(1,2),3),512,[],1024,1000);
            view(90,90);
            colormap jet;
            colorbar('delete');
            colorbar east Visible on;
...
for i=1:12
    ...
    hold on
    epoch=['e' num2str(i)];
    bar(1:6, [data_pro00073545_s026.trials.t1.psd.(epoch).sai.supermeans.beta(:)]);
    errorbar(1:6, [data_pro00073545_s026.plots.(lower(freq)).c1.ses(i)...
    data_pro00073545_s026.plots.(lower(freq)).c2.ses(i)...
    data_pro00073545_s026.plots.(lower(freq)).c3.ses(i)...
    data_pro00073545_s026.plots.(lower(freq)).c4.ses(i)...
    data_pro00073545_s026.plots.(lower(freq)).c5.ses(i)...
    data_pro00073545_s026.plots.(lower(freq)).c6.ses(i)]);
    hold off
end
