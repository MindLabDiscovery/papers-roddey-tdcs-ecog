...
class SynapseAPI:
    ...
    def formatErrorMsg(self, returnMsg):
        # _return_msg_ used to be a plain message, but the server now
        # sends a string containing serialized JSON. If it parses as
        # JSON, format the fields cleanly; otherwise return it as-is.
        if not isinstance(returnMsg, str) or len(returnMsg) == 0:
            return returnMsg
        if returnMsg.lstrip()[:1] != '{':
            return returnMsg
        try:
            parsed = json.loads(returnMsg)
        except:
            return returnMsg  # not valid JSON, leave the raw string
        if isinstance(parsed, dict):
            return ''.join('\n  {0}: {1}'.format(k, v) for k, v in parsed.items())
        return returnMsg
            ...
            if resp.status != 200:
                ...
                if retval is not None and '_return_msg_' in retval and len(retval['_return_msg_']) > 0:
                    print(' {0} {1} {2}'.format(
                                                ...
                                                self.formatErrorMsg(retval['_return_msg_']))
                    )
                retval = None
