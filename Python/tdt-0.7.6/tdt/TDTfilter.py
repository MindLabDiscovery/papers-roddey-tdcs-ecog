...
def epoc_filter(data, epoc, *, values=None, modifiers=None, t=None, tref=False, keepdata=True):
    ...
    if data.streams:
        for k in data.streams.keys():
            ...
            for j in range(time_ranges.shape[1]):
                ...
                # subtract as signed ints, uint64 wraps around when negative
                onset = np.uint64(max(np.round((int(tlo_sample) - int(td_sample))/sf),0))
                ...
                if np.isinf(time_ranges[1,j]):
                    ...
                    offset = np.uint64(max(np.round((int(thi_sample) - int(td_sample))/sf),0))
    ...
    if data.snips:
        ...
        for k in data.snips.keys():
            # snip ts can come back as an Nx1 column, flatten it so ts[j] is a scalar
            ts = np.ravel(data.snips[k].ts)
            ...
            for j in range(len(ts)):
                ...
                if len(ts_ind) > 0:
                    ...
                    diffs[keep_ind] = ts[j] - time_ranges[0, ts_ind[0]] + t1; # relative ts
                    keep_ind += 1
