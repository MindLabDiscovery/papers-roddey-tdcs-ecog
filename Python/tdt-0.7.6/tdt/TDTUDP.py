...
class TDTUDP():
    def __init__(self, host='10.1.0.100', send_type=np.int32, recv_type=np.int32, verbose=False,
...
if __name__ == '__main__':
    ...
    if 0:
        ...
        while 1:
            ct += 1
            start = ct % 10
            fakedata = np.arange(start, SEND_PACKETS + start, dtype=udp.send_type)
            if udp.send_type == np.float32:
                fakedata = fakedata * 2.
            udp.send(fakedata)
