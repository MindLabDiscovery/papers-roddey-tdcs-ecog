                ...
                if len(filter_ind[jj]) == 0:
                    ...
                    if current_type_str == 'streams':
                        ...
                        if len(ind2) > 0:
                            ...
                            # cast to int, unsigned chan dtype breaks np.arange
                            nchan = int(max(data[current_type_str][var_name].chan))
                            if ind2 - nchan >= -1:
            ...
            if current_name in sev_names:
                ...
                for jj in range(num_ranges):
                    ...
                    if export in ['csv', 'interlaced']:
                        ...
                        minSample = np.uint64(max(int(ltSample) - int(tdSample), 0))
                        if np.isinf(valid_time_range[1,jj]):
                            maxSample = MAX_UINT64
                        else:
                            etSample = time2sample(valid_time_range[1,jj], current_freq, t1=1)
                            maxSample = max(int(etSample) - int(tdSample) - 1, -1)
